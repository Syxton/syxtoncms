...
global $CFG, $MYVARS, $USER;
	...
	if ($discussions) {
		while ($discussion = fetch_row($discussions)) {
			...
			$content .= '	<tr>
						<td class="' . $viewclass . '"><span style="position:relative;float:left;max-width:95%;"><a href="javascript: void(0);" onclick="save_action($(this), $(\'#forum_refresh_' . $forumid . '\')); ajaxapi(\'/features/forum/forum_ajax.php\',\'get_forum_posts\',\'&discussionid=' . $discussion['discussionid'] . '&pagenum=0&catid=' . $catid . '&forumid=' . $forumid . '&pageid=' . $discussion["pageid"] . '\',function() {if (xmlHttp.readyState == 4) { simple_display(\'forum_div_' . $forumid . '\'); }}, true); " >' . $lock.stripslashes($discussion["title"]) . '</a><br />' . get_post_pages($forumid, $discussion, false, 3, false) . '</span>';
...
global $CFG, $MYVARS, $USER;
	...
	$returnme = '<div class="forum_breadcrumb">
                    <a href="javascript: void(0);"
                       onclick="save_action($(this), $(\'#forum_refresh_' . $forumid . '\'));
                                ajaxapi(\'/features/forum/forum_ajax.php\',
                                        \'get_forum_categories_ajax\',
                                        \'&forumid=' . $forumid . '&pageid=' . $pageid . '&catid=' . $catid . '\',
                                        function() {
                                            if (xmlHttp.readyState == 4) {
                                                simple_display(\'forum_div_' . $forumid . '\');
                                            }
	...
	while ($post = fetch_row($posts)) {
		...
		// DELETE POST
		if ($post["postid"] != $firstpost && user_is_able($USER->userid, "deleteforumpost", $pageid)) {
