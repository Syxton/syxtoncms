...
global $CFG, $USER, $ROLES;
    ...
    ajaxapi([
        'id'   => "online_users_$featureid",
        'url'  => '/features/onlineusers/onlineusers_ajax.php',
        'data' => [
            'action' => 'get_onlineusers_ajax',
            'pageid' => $pageid,
            'featureid' => $featureid,
        ],
        'display' => "onlineusersfeature",
        "event" => "none",
        "intervalid" => "online_users_$featureid",
        "interval" => 30000,
    ]);

    $content .= js_code_wrap(preg_replace('/\s+/S', " ", "online_users_$featureid();"));
...
global $CFG, $USER;
    ...
    if (is_logged_in()) {
        if ($pageid == $CFG->SITEID) {
            $SQL = "SELECT u.fname, u.lname, u.last_activity FROM users u INNER JOIN logfile lf ON lf.userid = u.userid AND lf.timeline = (SELECT timeline FROM logfile WHERE userid=u.userid ORDER BY timeline DESC LIMIT 1) WHERE u.userid IN (SELECT l.userid FROM logfile l WHERE l.timeline > $timelimit ORDER BY l.timeline DESC) AND lf.description != 'Logout' GROUP BY u.userid ORDER BY u.last_activity DESC";
        } else {
