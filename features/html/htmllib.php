        ...
        if(preg_match_all($regex, $html, $matches, PREG_SET_ORDER)){ //link to youtube
			foreach($matches as $match){
				...
				if(!strstr($match[0],'javascript:') && strlen($id) > 0){	
                    if(preg_match('/((http:\/\/)?(?:youtu\.be\/|(?:[a-z]{2,3}\.)?youtube\.com\/v\/)([\w-]{11}).*|http:\/\/(?:youtu\.be\/|(?:[a-z]{2,3}\.)?youtube\.com\/watch(?:\?|#\!)v=)([\w-]{11}).*)/i',$match[0]) || 
                    preg_match('/(\s*\.[yY][oO][uU][tT][uU][bB][eE]\.[cC][oO][mM][\/]\s*)/',$url)){
                        $size = $area == "middle" ? 'style="width:500px;height:300px;display:inline-block"' : '';
                        $html = str_replace($match[0], '<div class="aspect-ratio" '.$size.'><iframe src="//www.youtube.com/embed/'.$id.'" frameborder="0" allowfullscreen></iframe></div>',$html);    
                    }
				}
			}
		}
