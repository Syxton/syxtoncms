...
global $CFG, $USER;
	...
	if ($events = get_db_result($SQL)) {
		while ($event = fetch_row($events)) {
			...
			$returnme .= ' <a href="javascript: if (confirm(\'Are you sure you want to delete this event?\')) { ajaxapi(\'/features/events/events_ajax.php\',\'delete_events_relay\',\'&eventid=' . $event['eventid'] . '\',function() { update_login_contents(\'' . $pageid . '\');});}"> <img src="' . $CFG->wwwroot . '/images/delete.png" title="Delete Event" alt="Delete Event" /></a>';
...
global $CFG;
	...
	if (!empty($CFG->logofile)) {
		$email = '<img src="' . $protocol.$CFG->wwwroot . '/images/' . $CFG->logofile . '" style="max-width: 80%;" /><br />';
	} else {
