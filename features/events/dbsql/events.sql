...
insert_event||
    ...
    VALUES
    (||pageid||, ||template_id||, ||name||, ||category||, ||location||, ||allowinpage||, ||start_reg||, ||stop_reg||, ||max_users||, ||event_begin_date||,
...
registration_sorting_select||
    SELECT `value`
    FROM `events_registrations_values`
    WHERE `||elementname||` = '||elementvalue||'
    AND `regid` = v.`regid` LIMIT 1
||registration_sorting_select||
...
get_contacts_list||
    SELECT `name`, CONCAT(contactid, ":", `name`, ":", `email`, ":", `phone`) AS `contactdata`
...
confirmable_events||
    ...
    AND (
            (
                e.`pageid` != ||pageid|| AND `siteviewable` = 1
            )
            OR
            (
                e.`pageid` = ||pageid||
