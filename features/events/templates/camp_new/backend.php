	...
	if($regid = enter_registration($eventid, $reg, $MYVARS->GET["email"])){ //successful registration
		...
		if($event['fee_full'] != 0){
			...
			if($MYVARS->GET['payment_method'] == "PayPal"){ //Event paid by paypal
				echo '<div style="margin:auto;width:90%;text-align:center;"><h3>You have successfully added: '.$MYVARS->GET["Camper_Name_First"].' for '.$event['name'] . ' to your cart.</h3>';
                ...
                echo '<div style="margin:auto;width:90%;text-align:center;"><h3>You have successfully registered '.$MYVARS->GET["Camper_Name_First"].' for '.$event['name'] . '.</h3>';
                
                echo '<br />Your expected payment is <span style="color:blue;font-size:1.25em;">$'.number_format($MYVARS->GET["cart_total"],2).'</span>';
        ...
        echo '<br />' . facebook_share_button($eventid,$MYVARS->GET["Camper_Name_First"],$keys);
...
global $CFG,$USER,$PAGE;
	...
	$SQL = "SELECT e.* FROM events e WHERE $includelastevent (e.template_id=".$event["template_id"]." AND (e.pageid='".$event["pageid"]."' $siteviewable)) AND (e.start_reg < $time AND e.stop_reg > $time) AND (e.max_users=0 OR (e.max_users != 0 AND e.max_users > (SELECT COUNT(*) FROM events_registrations er WHERE er.eventid=e.eventid AND verified='1')))";
