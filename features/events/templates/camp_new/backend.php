...
//error_reporting(E_ERROR | E_PARSE); // Keep warnings from showing
	...
	//Check for errors
	$error = "";
	if ($regid = enter_registration($eventid, $reg, $email, $pending)) { // Successful registration
		...
		if ($event['fee_full'] != 0) {
			...
			if ($payment_method !== "Campership") {
				echo '  <h3>You have successfully added: ' . $Camper_Name_First . ' for ' . $event['name'] . ' to your cart.</h3>';
				echo '  <br />Your current cart total is:  <span style="color:blue;font-size:1.25em;">$' . number_format($cart_total, 2) . '</span>';
			} else { // Campership
				echo '  <h3>You have successfully registered: ' . $Camper_Name_First . ' for ' . $event['name'] . '.</h3>';
			}
			...
			if ($more1 || $more2) {
				...
				echo empty($more1) ? "" : '<strong>Register ' . $Camper_Name_First . ' for another week.<br />Select from the weeks available</strong><br />' . $more1 . '.<br />';
		...
		echo '</div>
				<br />
				<div style="margin:auto;width:90%;text-align:center;">' . facebook_share_button($event, $Camper_Name_First) . '</div>';
