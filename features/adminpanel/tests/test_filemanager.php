...
if (!function_exists('curl_init')) {
    ...
    /**
     * Returns true when the HTTP response is a ModSecurity-generated 406.
     *
     * Production may reject obvious path-traversal requests before api.php
     * receives them. This is still a valid security rejection, but it means
     * the application cannot return its normal 400 response.
     */
    function fm_is_modsecurity_rejection(int $code, string $rawBody): bool {
        if ($code !== 406) {
            return false;
        }

        return stripos($rawBody, 'generated by Mod_Security') !== false;
    }
    ...
    [$code, $data, $raw] = fm_test_call($apiUrl, $cookieHeader, $isLocalHost, $base + [
        'action' => 'list', 'path' => '../../../../etc',
    ]);
    $traversalRejected = $code === 400 || fm_is_modsecurity_rejection($code, $raw);
    $tests2 .= testCheck("security: path traversal in 'path' is rejected", $traversalRejected ? "PASS" : "FAIL: HTTP $code " . json_encode($data), $passCounter2, $totalCounter2);
