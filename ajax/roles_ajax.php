...
global $CFG,$ROLES,$USER,$MYVARS;
	...
	if($MYVARS->GET["refreshroles"] == "refreshroles"){
		$returnme = '<div style="width:100%; text-align:center;"><select size="10" width="100%" style="width: 100%; font-size:.85em;" name="userid" id="'.$type.'user_select" onclick="if(document.getElementById(\''.$type.'user_select\').value > 0){ajaxapi(\'/ajax/roles_ajax.php\',\'refresh_user_roles\',\'&amp;pageid='.$pageid.'&amp;userid=\'+document.getElementById(\''.$type.'user_select\').value,function(){ simple_display(\''.$type.'roles_div\'); });}">';
	}else{
		$returnme = '<div style="width:100%; text-align:center;"><select size="10" width="100%" style="width: 100%; font-size:.85em;" name="userid" id="'.$type.'user_select" onclick="if(document.getElementById(\''.$type.'user_select\').value > 0){ajaxapi(\'/ajax/roles_ajax.php\',\'refresh_user_abilities\',\'&amp;pageid='.$pageid.'&amp;feature='.$feature.'&amp;featureid='.$featureid.'&amp;userid=\'+document.getElementById(\''.$type.'user_select\').value,function(){ simple_display(\''.$type.'abilities_div\'); });}">';
	}
