...
function ajaxapi(script, action, param, display, async){
	...
	if(async != true){
    	...
    	display();
	}else{ ajaxpost(url,parameters,true,display); }
}
...
function refreshajax(script, action, param, display, async) {
	if (xmlHttp.readyState != 4 && xmlHttp.readyState != 0){
		setTimeout(function(){ajaxapi(script, action, param, display, async); },1000); //if there is an ajax conflict.  Wait 1 second before trying again.
...
function hide_show_buttons(section_name,block){ var display = block == true ? "block" : "inline"; document.getElementById(section_name).style.display = document.getElementById(section_name).style.display=='none' || document.getElementById(section_name).style.display.length==0 ? display : 'none'; }
...
function checkPassword(x,y,f,alertsoff) {
    if(x.value != y.value){
        if(!alertsoff)alert("The entered password fields must match");
        x.value = "";
        y.value = "";
        f.focus();
