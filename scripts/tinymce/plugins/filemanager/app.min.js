...
try { window.localStorage.setItem('fm_' + key, value); } catch (e) { }
}
...
if (loc.userid !== USERID) return null; 
if (Date.now() - loc.ts > LAST_LOC_MAX_AGE_MS) return null; 
return loc;
} catch (e) { return null; }
}
...
sortBy: loadPref('sortBy', 'date'), 
sortDir: loadPref('sortDir', 'desc'), 
query: '', 
};
...
if (state.area === 'old') return false; 
var menuItems = [];
...
state.area = area;
state.id = area === 'pub' ? PAGEID : USERID; 
state.path = '';
state.selected = null;
state.query = '';
clearMultiSelect();
...
if (toPath === state.path) return false; 
...
if (!onConflict) { confirmBtn.disabled = false; return; } 
...
if (state.area === area && state.id === id) load(); 
refresh();
}));
});
}).catch(function (err) {
...
if (!menuItems.length) return wrap; 
...
try { e.dataTransfer.setData('text/plain', opts.name); } catch (e2) { }
try {
var key = multiKey(opts.name, opts.isFolder);
var items;
...
var finalName = res.body.name || newName; 
...
if (state.area === 'old' || !pubAreaOK(PERM.upload)) return;
if (uploadState || uploadPending) return; 
...
uploadPending = false;
if (!onConflict) { renderToolbar(); return; } 
