...
function htmLawed($t, $C=1, $S=[])
{
  ...
  if (!isset($C['schemes']['*'])) {
	$C['schemes']['*'] = ['file'=>1, 'http'=>1, 'https'=>1, 'ws'=>1, 'wss'=>1];
	if (empty($C['safe'])) {
  ...
  $C['cdata'] = $C['cdata'] ?? (empty($C['safe']) ? 3 : 0);
  $C['clean_ms_char'] = empty($C['clean_ms_char']) ? 0 : $C['clean_ms_char'];
  $C['comment'] = $C['comment'] ?? (empty($C['safe']) ? 3 : 0);
  ...
  if ($C['show_setting'] && preg_match('`^[a-z][a-z0-9_]*$`i', $C['show_setting'])) {
	$GLOBALS[$C['show_setting']] = ['config'=>$C, 'spec'=>$S, 'time'=>microtime(true), 'version'=>hl_version()];
  }
  unset($C, $eleAr);
  if (isset($oldC)) {
	$GLOBALS['C'] = $oldC;
...
function hl_attributeValue($attr, $value, $ruleAr, $ele)
{
  ...
  return (isset($out[0]) ? $out : ($ruleAr['default'] ?? 0));
}
...
function hl_balance($t, $act=1, $parentEle='div')
{
  ...
  $closingTagOmitableEleAr = ['caption'=>1, 'colgroup'=>1, 'dd'=>1, 'dt'=>1, 'li'=>1, 'optgroup'=>1, 'option'=>1, 'p'=>1, 'rp'=>1, 'rt'=>1, 'tbody'=>1, 'td'=>1, 'tfoot'=>1, 'th'=>1, 'thead'=>1, 'tr'=>1];
  ...
  $otherValidMomKidAr = ['address'=>['p'=>1], 'applet'=>['param'=>1], 'audio'=>['source'=>1, 'track'=>1], 'blockquote'=>['script'=>1], 'fieldset'=>['legend'=>1, '#pcdata'=>1],  'figure'=>['figcaption'=>1],'form'=>['script'=>1], 'map'=>['area'=>1], 'legend'=>['h1'=>1, 'h2'=>1, 'h3'=>1, 'h4'=>1, 'h5'=>1, 'h6'=>1], 'object'=>['param'=>1, 'embed'=>1], 'ruby'=>['rb'=>1, 'rbc'=>1, 'rp'=>1, 'rt'=>1, 'rtc'=>1], 'summary'=>['h1'=>1, 'h2'=>1, 'h3'=>1, 'h4'=>1, 'h5'=>1, 'h6'=>1, 'hgroup'=>1], 'video'=>['source'=>1, 'track'=>1]];
...
function hl_tag($t)
{
  ...
  static $deprecatedEleAr = ['acronym'=>1, 'applet'=>1, 'big'=>1, 'center'=>1, 'dir'=>1, 'font'=>1, 'isindex'=>1, 's'=>1, 'strike'=>1, 'tt'=>1];
  ...
  static $emptyEleAr = ['area'=>1, 'br'=>1, 'col'=>1, 'command'=>1, 'embed'=>1, 'hr'=>1, 'img'=>1, 'input'=>1, 'isindex'=>1, 'keygen'=>1, 'link'=>1, 'meta'=>1, 'param'=>1, 'source'=>1, 'track'=>1, 'wbr'=>1];
  if (!empty($m[1])) {
	return(
  ...
  static $urlAttrAr = ['action'=>1, 'archive'=>1, 'cite'=>1, 'classid'=>1, 'codebase'=>1, 'data'=>1, 'formaction'=>1, 'href'=>1, 'itemtype'=>1, 'longdesc'=>1, 'model'=>1, 'pluginspage'=>1, 'pluginurl'=>1, 'poster'=>1, 'src'=>1, 'srcset'=>1, 'usemap'=>1]; // Excludes style and on*
  ...
  static $requiredAttrAr = ['area'=>['alt'=>'area'], 'bdo'=>['dir'=>'ltr'], 'command'=>['label'=>''], 'form'=>['action'=>''], 'img'=>['src'=>'', 'alt'=>'image'], 'map'=>['name'=>''], 'optgroup'=>['label'=>''], 'param'=>['name'=>''], 'style'=>['scoped'=>''], 'textarea'=>['rows'=>'10', 'cols'=>'50']];
  ...
  if ($C['xml:lang'] && isset($filtAttrAr['lang'])) {
	$filtAttrAr['xml:lang'] ??= $filtAttrAr['lang'];
	if ($C['xml:lang'] == 2) {
...
function hl_tidy($t, $format, $parentEle)
{
  ...
  $t =
	preg_replace(
		...
		[' $1', ' ', '$1'],
		preg_replace_callback(
			['`(<(!\[CDATA\[))(.+?)(\]\]>)`sm', '`(<(!--))(.+?)(-->)`sm', '`(<(pre|script|textarea)[^>]*?>)(.+?)(</\2>)`sm'],
			'hl_aux2',
			$t
		)
	);
  ...
  if (($format = strtolower($format)) == -1) {
	return
		str_replace(["\x01", "\x02", "\x03", "\x04", "\x05", "\x07"], ['<', '>', "\n", "\r", "\t", ' '], $t);
  }
...
function hl_url($url, $attr=null)
{
  ...
  $okSchemeAr = $C['schemes'][$attr] ?? $C['schemes']['*'];
