...
if (cardField.isEligible()) {
    const nameField = cardField.NameField({
        style: { input: { color: "grey" }, ".invalid": { color: "green" } },
    });
...
async function createOrderCallback() {
    resultMessage("");
    try {
        const response = await fetch("./backend/index.php?action=orders", {
            method: "POST",
            headers: {
...
async function onApproveCallback(data, actions) {
    try {
        const response = await fetch(`./backend/index.php?action=capture&orderID=${data.orderID}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
        ...
        if (errorDetail || !transaction || transaction.status === "DECLINED") {
            ...
            // Find 1st iframe or top frame if no iframe is found.
            let iframe = top.document.querySelector("iframe.cboxIframe");
            let domain = window.location.origin;
            let rootpath = typeof dirfromroot != "undefined" ? '/' + dirfromroot + '/' : (typeof top.window.dirfromroot != "undefined" ? '/' + top.window.dirfromroot + '/' : false);

            if (rootpath == false) {
                console.log("rootpath is false");
                return false;
            }

            var url = domain + rootpath + "scripts/payments/backend/approved.php";
...
function goToUrlWithJson(url, jsonObject) {
    let jsonString = JSON.stringify(jsonObject);
    let encodedJson = encodeURIComponent(jsonString);
    return `${url}?data=${encodedJson}`;
}
