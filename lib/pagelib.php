    ...
    if (empty($_POST["aslib"])) {
        ...
        if (function_exists($action)) {
            try {
                ...
                debugging("Error in function '$action': " . $e->getMessage());
            }
        } else {
            debugging(error_string("no_function", [$action]), 2);
        }
    }
}
...
function action_path($filename, $ajaxfolder = false) {
global $CFG;
    if ($ajaxfolder) {
        return $CFG->wwwroot . "/ajax/$filename.php?action=";
    } else {
        $feature = str_replace("_ajax", "", $filename);
        return $CFG->wwwroot . "/features/$feature/$filename.php?action=";
    }
}
