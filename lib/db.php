...
global $CFG;
    ...
    $thisversion = 20181106; //Add debug field to site logs
    if ($version < $thisversion) {
        $SQL = "ALTER TABLE `menus` ADD `parent` INT NULL DEFAULT NULL AFTER `hidefromvisitors`;";
        if (execute_db_sql($SQL)) {
            execute_db_sql("UPDATE settings SET setting='$thisversion' WHERE type='site' AND setting_name='version'");
        }
    }

    $thisversion = 20240819.02;
    if ($version < $thisversion) {
        try {
            start_db_transaction();
            add_role_ability('roles', 'removeownrole', 'Roles', '2', 'Can remove own role from page', '1', '1', '1', '1');
