...
global $MYVARS, $CFG;
    		...
    		<form id="signup_form">
    			<fieldset class="formContainer">
    				<div class="rowContainer">
    					<label for="email">Email Address</label><input type="text" id="email" name="email" data-rule-required="true" data-rule-email="true" data-rule-ajax1="ajax/site_ajax.php::unique_email::&email=::true" data-msg-required="'.get_error_message('valid_req_email').'" data-msg-email="'.get_error_message('valid_email_invalid').'" data-msg-ajax1="'.get_error_message('valid_email_unique').'" /><div class="tooltipContainer info">'.get_help("input_email").'</div><br />
    				</div>
    				<div class="rowContainer">
    					<label for="fname">First Name</label><input type="text" id="fname" name="fname" data-rule-required="true" data-msg-required="'.get_error_message('valid_req_fname').'" /><div class="tooltipContainer info">'.get_help("input_fname").'</div><br />
    				</div>
    				<div class="rowContainer">
    					<label for="lname">Last Name</label><input type="text" id="lname" name="lname" data-rule-required="true" data-msg-required="'.get_error_message('valid_req_lname').'" /><div class="tooltipContainer info">'.get_help("input_lname").'</div><br />
    	  			</div>
    			  	<div class="rowContainer">
    			  		<label for="mypassword">Password</label><input type="password" id="mypassword" name="mypassword" data-rule-required="true" data-rule-minlength="6" data-msg-required="'.get_error_message('valid_req_password').'" data-msg-minlength="'.get_error_message('valid_password_length').'" /><div class="tooltipContainer info">'.get_help("input_password").'</div><br />
    	  			</div>
    			  	<div class="rowContainer">
    				  	<label for="vpassword">Verify Password</label><input type="password" id="vpassword" name="vpassword" data-rule-required="true" data-rule-equalTo="#mypassword" data-msg-required="'.get_error_message('valid_req_vpassword').'" data-msg-equalTo="'.get_error_message('valid_vpassword_match').'" /><div class="tooltipContainer info">'.get_help("input_vpassword").'</div><br/>
    	  			</div>
    ...
    echo format_popup($content,$CFG->sitename.' Signup',"255px");
}
...
global $MYVARS, $CFG;
	...
	if(get_db_row("SELECT * FROM users WHERE userid=$userid AND alternate='$alternate'")){
        ...
        echo format_popup($content,'Change Password',"255px");   
    }else{
	   ...
	   $userid = $USER->userid;
		...
		<div id="change_profile">
            ...
            <form id="profile_change_form">
				<fieldset class="formContainer">
				  	<div class="rowContainer">
			  			<label for="myfname">First Name</label><input value="'.$USER->fname.'" type="text" id="myfname" name="myfname" data-rule-required="true" /><div class="tooltipContainer info">'.get_help("input_fname").'</div><br />
                    </div>
                    <div class="rowContainer">
			  			<label for="mylname">Last Name</label><input value="'.$USER->lname.'" type="text" id="mylname" name="mylname" data-rule-required="true" /><div class="tooltipContainer info">'.get_help("input_fname").'</div><br />
                    </div>
                    <div class="rowContainer">
					   <label for="email">Email Address</label><input type="text" value="'.$USER->email.'" id="email" name="email" data-rule-required="true" data-rule-email="true" data-rule-ajax1="ajax/site_ajax.php::unique_email::&email=::true::'.$USER->email.'" data-msg-required="'.get_error_message('valid_req_email').'" data-msg-email="'.get_error_message('valid_email_invalid').'" data-msg-ajax1="'.get_error_message('valid_email_unique').'" /><div class="tooltipContainer info">'.get_help("input_email").'</div><br />
				    </div>
 				  	<div class="rowContainer">
			  			<label for="mypassword">Password</label><input type="password" id="mypassword" name="mypassword" data-rule-minlength="6" data-msg-minlength="'.get_error_message('valid_password_length').'" /><div class="tooltipContainer info">'.get_help("input_password").'</div><br />
                    </div>
    			  	<div class="rowContainer">
					  	<label for="vpassword">Verify Password</label><input type="password" id="vpassword" name="vpassword" data-rule-equalTo="#mypassword" data-msg-equalTo="'.get_error_message('valid_vpassword_match').'" /><div class="tooltipContainer info">'.get_help("input_vpassword").'</div><br/>
		  			</div>
			  		<input class="submit" name="submit" type="submit" value="Save" style="margin: auto;width: 80px;display: block;" />	
				</fieldset>
			</form>
        ...
        echo format_popup($content,'Edit Profile',"255px");
	}else{
		echo '<script type="text/javascript">go_to_page(1);</script>';
	}
}
...
global $MYVARS, $CFG;
	...
	Please type the email address that is associated with your user account.  A new temporary password will be sent to this address.  You will then be able to log into the website and change your password.<br /><br />
		<form id="password_request_form">
			<fieldset class="formContainer">
				<div class="rowContainer">
					<label for="email">Email Address</label><input type="text" id="email" name="email" data-rule-required="true" data-rule-email="true" data-rule-ajax1="ajax/site_ajax.php::unique_email::&email=::false" data-msg-required="'.get_error_message('valid_req_email').'" data-msg-email="'.get_error_message('valid_email_invalid').'" data-msg-ajax1="'.get_error_message('valid_email_used').'" /><div class="tooltipContainer info">'.get_help("input_email").'</div><br />
				</div>
    ...
    echo format_popup($content,'Forgot Password',"255px");
}
